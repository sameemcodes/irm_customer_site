...
def index(request):
    destpath = os.path.join("customer-updation-templates", "index.html")
...
def updation(request):
    if request.method == 'POST':
        ...
        if IrmCustGblMst.objects.filter(cust_legal_name = cid).exists():
            print("yeshhhh")
            customer = IrmCustGblMst.objects.get(cust_legal_name = cid)
            return render(request,'customer-updation-templates/update_portal.html',{"customer" : customer} )
            

        else:
            request.session['login_status'] = True
            return JsonResponse({"No Such customer Id":True}, status=200)

    return render(request, 'customer-updation-templates/search_customer.html')

def update_header_data_db(IrmCustGblMst_db_data):
    cid = IrmCustGblMst_db_data.POST.get('cust_id')
    customer = IrmCustGblMst.objects.get(cust_id=cid)
    customer.cust_legal_name = IrmCustGblMst_db_data.POST.get('cust_legal_name')
    customer.deploy_type_mst_id = IrmCustGblMst_db_data.POST.get('deploy_type_mst_id')
    customer.audit_type_mst = IrmCustGblMst_db_data.POST.get('audit_type_mst')
    customer.country= IrmCustGblMst_db_data.POST['country']
    customer.upd_date = date.today()
    customer.save()
    return render(IrmCustGblMst_db_data, 'customer-updation-templates/index.html')